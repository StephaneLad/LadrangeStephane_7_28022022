      ...
      bcrypt.hash(req.body.password, 10).then((hash) => {
        userModel
          ...
          .then((user) => {
            ...
            res.status(200).json({
              userId: user.id_user,
              token: jwt.sign({ userId: user.id_user }, "clef", {
                expiresIn: "24h",
              }),
              is_admin: false,
            });
        ...
        res.status(400).json({message:"email allready use"})
    }
  })      
};
      ...
      bcrypt
        ...
        .then((valid) => {
          if (valid !== true) {
            ...
            return res.status(400).json({ message: "invalid password",passwordError });
          }
          if (user.is_admin === 1) {
            res.status(200).json({
              userId: user.id_user,
...
exports.update = (req, res) => {
  ...
  const atMail = "@";
  userModel
    .findOne({ where: { id_user: `${data.id_user}` } })
    .then((user) => {
      if (!user) {
...
exports.delete = (req, res) => {
  userModel
    .destroy({ where: { id_user: `${req.body.id_user}` } })
